...
`audit` infrastructure introduced by feature `001`.
...
isolation; tokens strictly scoped to their owning customer.
...
| IV | Masked logs; audit on state change; actionable errors | Masked logging; audit on delete (FR-014); distinguishable errors (FR-013). | PASS |
| V | Simplest design; **no invented conveniences** | The decisive gate here. The library exposes no `tokenize` method because BML has no such endpoint, even though callers will expect one. A unit test enforces the absence (SC-006). | PASS |
